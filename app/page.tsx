    ...
    <div className="group relative overflow-hidden rounded-2xl border border-white/10 bg-zinc-900/50 transition-all hover:border-white/20">
      ...
      <div className="absolute bottom-0 left-0 p-8 w-full">
        <div className="flex items-center gap-3 mb-4">
          <span className="px-2 py-0.5 rounded bg-white text-black text-[10px] font-bold uppercase tracking-tighter">
            Featured
          </span>
...
function NewsCard({ item }: { item: NewsItem }) {
  return (
    <div className="group flex flex-col rounded-xl border border-white/10 bg-zinc-900/30 transition-all hover:border-white/20 hover:bg-zinc-900/50 p-5">
      ...
      <p className="text-zinc-400 text-sm mb-6 line-clamp-3 leading-relaxed flex-1">{item.summary}</p>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          ...
          <div className="flex items-center gap-4">
            <div className="flex flex-col items-end">
              <span className="text-[10px] font-mono uppercase text-zinc-500 tracking-widest mb-1">
                Last Update
              </span>
              <span className="text-sm font-medium text-white">May 06, 2026</span>
            </div>
            <Link href="/chat" className="px-6 py-3 rounded-xl bg-white text-black font-bold transition-all hover:bg-zinc-200 active:scale-95">
              Ask AI Assistant
            </Link>
          </div>
        </div>
          ...
          {newsCountries.map((c) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-medium transition-all ${
                selectedCountry === c.code ? "bg-white text-black" : "text-zinc-400 hover:text-white hover:bg-white/5"
              }`}
            >
              {c.flag} {c.name}
            </button>
        ...
        <div className="flex flex-wrap items-center gap-2 border-y border-white/5 py-6">
          {newsTags.map((tag) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-medium transition-all ${
                selectedTag === tag ? "bg-white text-black" : "text-zinc-400 hover:text-white hover:bg-white/5"
              }`}
            >
